...
class NewClass3 implements ActionListener {
	/*
		...
		if (ae.getActionCommand() == "Insert") {
			...
			try {
				Connection con = DriverManager
						.getConnection("jdbc:sqlserver://11008-FAC2;databaseName=adventureworks;user=sa;password=1");
		...
		if (ae.getActionCommand() == "Update") {
			try {
				...
				stat = con.prepareStatement(
						"UPDATE publishers SET pname = ?,pphone= ?,padd=? , pcity = ?, pstate = ? ,pzip=? where pid = ?");
				String aid = au_idField.getText();
				String aname = au_nameField.getText();
				String aphone = phoneField.getText();
				String aadd = addressField.getText();
