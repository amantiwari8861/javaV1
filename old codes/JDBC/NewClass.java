...
{
/*Declare the variables to create the application interface for the Publishers 
Information application*/
Label heading, lauid, lau_name, lphone, laddress, lcity, lstate,lzip,error;
TextField au_idField, au_nameField, phoneField, 
addressField,cityField,stateField,zipField;
...
Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","MY_DEMO","33");
   	/*Create a Statement object*/
   	stmt = con.createStatement();
        con.setAutoCommit(false);
        stat = con.prepareStatement("INSERT INTO MY_DEMO.PUBLISHERS VALUES(?, ?, ?, ?, ?,?,?)");
...
("jdbc:oracle:thin:@localhost:1521:xe","MY_DEMO","33");
stat = con.prepareStatement("UPDATE MY_DEMO.PUBLISHERS SET pname = ?,pphone= ?,padd=?,pcity = ?, pstate = ? ,pzip=? where pid = ?");
        String aid = au_idField.getText();
        String aname = au_nameField.getText();
  	String aphone = phoneField.getText();
  	String aadd = addressField.getText(); 
	...
	int result=stat.executeUpdate();
        System.out.println("Number of rows upadted"+ " "+result);
	con.close();
       }
  	catch(Exception e)
            {
